...
def aesEncryption(PUBLIC_KEY):
    ...
    while True:
        ...
        clientSocket.send(encryption.encrypt("this procns project is a piece of golde & a past"))
        printStatistics()
...
def blowfishEncryption(PUBLIC_KEY):
    ...
    while True:
        ...
        clientSocket.send(bf.cryptBlowfish(PUBLIC_KEY,"this procns project is a piece of golde & a past"))
        printStatistics()
...
def rsaEncryption(message):
    ...
    public, private = rsa.generatePQ()
    signedMessage = rsa.encrypt(private, message)
    socketDict['RSASocket'].listen(5)
    while True:
        clientSocket,clientAddress = socketDict['RSASocket'].accept()
        print 'the client address is ',clientAddress
        clientSocket.send(pickle.dumps(public))
        clientSocket.send(pickle.dumps(signedMessage))
        printStatistics()
        socketDict['RSASocket'].close()
        break;
...
for index in socketConfig:
    ...
    socketDict[index].setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
