...
def requestRSA(message):

    print "message is ",message
    socketDict['RSASocket'].connect(connectConfig[socketConfig['RSASocket']]);
    public = pickle.loads(socketDict['RSASocket'].recv(2048))
    encryptedMessage = pickle.loads(socketDict['RSASocket'].recv(1024))

    if rsa.decrypt(public, encryptedMessage) == message:
        print 'signature has been verified'
    else:
        print 'signature verification has failed'
